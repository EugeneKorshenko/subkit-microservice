...
```
curl -X GET -H "api_Key: 6654edc5-82a3-4006-967f-97d5817d7fe2" -G --data-urlencode 'limit=10' --data-urlencode 'filter={"value.metadata.Type":"MPNS"}' http://localhost:8080/stores/identities | python -m json.tool

curl -X GET -H "api_Key: 6654edc5-82a3-4006-967f-97d5817d7fe2" -G --data-urlencode 'limit=10' --data-urlencode 'filter={"$not":{ "value.password":"ofT6IM3I"}}' http://localhost:8080/stores/identities | python -m json.tool
```

Query parameter
===
```?filter={"value.FirstName":"John Doe"}```
