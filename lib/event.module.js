...
module.exports.init = function(conf, store){
	...
	function callWebHook(error, data, clientId, apiKey){
		...
		client.post('/manage/permissions/demo1', data || error, function(error, req, res){
			if(error) return console.log('Error WebHook call to: ' + clientId + ' Error: ' + error);
			console.log('Successful WebHook call to: ' + clientId + ' Status code: ' + res.statusCode);
		});		
	}
