...
*/
module.exports = function (store, hook) {
	module.exports.init(store, hook);
};

module.exports.init = function(store, hook){
	...
	hook.on(function(data){
		...
		if(exclusions.indexOf(data.$channel) === -1) {
			store.upsert('eventsource', data.$key, data.$payload, function(error){});
		}
	});
	...
	var _fromStreams = function(streams, eventStore){
		...
		streams.forEach(function(itm){
			readChain.push(function(err, stream){
				...
				store.query(eventStore, readOptions, {}, function(err, data){
