...
module.exports.init = function(server, filesModule, helper){
	//files
	//curl localhost:8080/files
	server.get("/files", helper.apiAuth, function(req,res,next){
		fs.readdir(filesModule.filesPath, function(error, data){
			var result = [];
	...
	server.post("/file/upload", helper.userAuth, function (req, res, next) {
		if(!req.files) return res.send(400);

		var fileName = req.files.filedata.name,
			oldPath = req.files.filedata.path,
			newPath = path.join(filesModule.filesPath, fileName);
	...
	server.post("/file/upload/:name", helper.userAuth, function (req, res, next) {
		if(!req.params.name) return res.send(400);
		
		var fileName = req.params.name,
			file = req.body.toString(),
			filePath = path.join(filesModule.filesPath, fileName);
	...
	server.put("/file/upload/:name", helper.userAuth, function (req, res, next) {
		var fileName = req.params.name,
			file = req.body.toString();
		if(!fileName) return res.send(400);

		var filePath = path.join(filesModule.filesPath, fileName);
		if(file > 100000){
			return res.send(400);
		}
		fs.writeFile(filePath, file, function(error, data){
	...
	server.get("/file/download/:name", helper.userAuth, function (req, res, next) {
		var fileName = req.params.name
			filePath = path.join(filesModule.filesPath, fileName);
	...
	server.del("/file/:name", helper.userAuth, function(req, res, next){
		var fileName = req.params.name,
			filePath = path.join(filesModule.filesPath, fileName);
