...
module.exports.init = function(server, hooks, doc){
	...
	(function(){
		...
		setInterval(function(){
			count++;
			hooks.publish('heartbeat', count, {value: count});		
		}, 1000);
	})();
	...
	server.get('/hooks/subscribe/:channel/:clientId', function (req, res, next) {
		...
		hooks.subscribe(channel, clientId, function(err, data){
	...
	server.del('/hooks/subscribe/:channel/:clientId', function(req,res,next){
		...
		hooks.unsubscribe(channel, clientId);
		res.send(202);
	});

	server.get('/hooks/channels', function(req,res,next){
		hooks.getChannels(function(err, data){
			res.send(200, data);
		});
	});
	server.get('/hooks/channels/:clientId', function(req,res,next){
		...
		hooks.getChannelsByClientId(clientId, function(err, data){
			if(err) res.send(404, err);
			res.send(200, data);
		});
	});
	...
	server.get('/hooks/clients/:channel', function(req,res,next){
		...
		hooks.getClientsByChannel(channel, function(err, data){
			if(err) res.send(404, err);
			res.send(200, data);
		});
	});
	...
	server.post('/hooks/client/publish/:clientId', function (req, res, next) {
		...
		if(message) hooks.send(clientId, message);
	...
	server.get('/hooks/client/:channel/:clientId', function (req, res, next) {
		...
		hooks.receive(channel, clientId, function(err, data){
			if(err) res.send(404, err);
			res.send(200, data);
		});
	});
	server.get('/hooks/client/:clientId', function (req, res, next) {
		...
		hooks.receiveAll(clientId, function(err, data){
			if(err) res.send(404, err);
			res.send(200, data);
		});
	});
