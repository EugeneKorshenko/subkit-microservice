...
module.exports.init = function(server, storage, identity, helper){
	...
	server.get("/identities/settings", function(req, res, next){
		res.send(200, {});
	});
	...
	//groups
	server.get("/identities/groups", function(req, res, next){
	...
	server.get("/identities/groups/users", function(req, res, next){
		storage.read("identities", {}, function(err, data){
			if(err) return res.send(500, err);
			var groups = data.reduce(function(state, event){
				var userGroups = event.value.groups || [];
	...
	server.get("/identities/groups/:groupId", function(req, res, next){
	...
	//identity
	server.get("/identities/:identityId", function(req, res, next){
		var identityId = req.params.identityId;
	...
	server.get("/identities", function(req, res, next){
	...
	server.post("/identities", function(req, res, next){
		identity.create(function(err, data){
			if(err) return res.send(500, err);
			res.send(201, { status: "created" });
		});
	});
	server.post("/identities/:identityId", function(req, res, next){
		var identityId = req.params.identityId;
	...
	server.put("/identities/:identityId", function(req, res, next){
		var identityId = req.params.identityId;
	...
	server.del("/identities/:identityId", function(req, res, next){
		var identityId = req.params.identityId;
