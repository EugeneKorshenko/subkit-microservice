...
module.exports.init = function(server, location, helper, doc){
	var location_doc = doc("location", "Location operations.");
	...
	server.post("/location", helper.apiAuth, function(req, res, next){
		var geoPoint = req.body || new location.GeoPoint(0,0);

		location.set(null, geoPoint, function(error, data){
			if(error) return next(error);
