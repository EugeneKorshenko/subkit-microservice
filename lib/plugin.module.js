...
module.exports.init = function(pluginContext, logger){
	...
	function load(pluginName, pluginVersion){
		...
		try {
			...
			logger.log('plugin', {
				type: 'plugin',
				status: 'success',
				message: 'Plugin: ' + pluginName + ' (' + pluginVersion + ') loaded.'
			});
		}
		catch(e){
			logger.log('plugin', {
				...
				status: 'error',
				error: e,
				message: 'Plugin Error: ' + pluginName + ' (' + pluginVersion + ') could not be loaded.'
			});
		}
	}
