...
module.exports.init = function(server, pubsub, doc){
	...
	//pubsub
	server.get('/pubsub/subscribe/:channel/:clientId', function (req, res, next) {
	...
	server.del('/pubsub/subscribe/:channel/:clientId', function(req,res,next){
		var channel = req.params.channel,
			clientId = req.params.clientId;
	...
	server.get('/pubsub/channels', function(req,res,next){
		pubsub.getChannels(function(err, data){
			res.send(200, data);
		});
	});
	server.get('/pubsub/channels/:clientId', function(req,res,next){
	...
	server.post('/pubsub/channel/publish/:channel', function (req, res, next) {
		var channel = req.params.channel,
			message = req.body;

		if(message) pubsub.publish(channel, message);
		res.send(201, {status: 'published'});
	});

	server.get('/pubsub/clients', function(req,res,next){
		pubsub.getClients(function(err, data){
			if(err) res.send(404, err);
			res.send(200, data);
		});
	});
	server.get('/pubsub/clients/:channel', function(req,res,next){
		var channel = req.params.channel;
	...
	server.post('/pubsub/client/publish/:clientId', function (req, res, next) {
		var clientId = req.params.clientId,
			message = req.body;
		
		if(message) pubsub.send(clientId, message);
		res.send(201, {status: 'sent'});
	});
	server.get('/pubsub/client/:channel/:clientId', function (req, res, next) {
		var clientId = req.params.clientId,
	...
	server.get('/pubsub/client/:clientId', function (req, res, next) {
		var clientId = req.params.clientId;
