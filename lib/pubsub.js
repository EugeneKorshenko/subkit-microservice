module.exports.init = function(server, pubsub, storage, es, helper){
	...
	(function(){
		...
		setInterval(function(){
			count++;
			pubsub.publish("heartbeat", {key:"heartbeat!"+count, value:count}, function(){});		
		}, 1000);
	})();
	...
	storage.onChange(function(data){
		var joined = "";
		data.key.split("!").forEach(function(key){
			if(joined === "") joined += key;
			else joined += "_" + key;
			pubsub.publish(joined, data, function(){});
		});
	});
	...
	server.get("/subscribe/:channel/:clientId", helper.apiAuth, function (req, res, next) {
		...
		pubsub.on(channel, clientId, function(error, data){
