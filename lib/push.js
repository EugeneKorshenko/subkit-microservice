...
module.exports.init = function(server, config, push, identity, helper){
	...
	server.post("/push/send", function(req, res, next){
		var device = req.body;
		if(device) {
			if(device.Type === "MPNS" || device.Type === "mpns") push.sendMPNS(device.Token, device.Message);
			if(device.Type === "APN" || device.Type === "apn") push.sendAPN(device.Token, device.Message);
