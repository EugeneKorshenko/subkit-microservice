...
module.exports.init = function(server, config, push, identity, helper){
	...
	//MPNS http://am3.notify.live.net/throttledthirdparty/01.00/AQHaXMAW1mZbRoHFxQuiQ4X2AgAAAAADAwAAAAQUZm52OjlDMTM0RTBBNkJDQ0U2MUUFBkVVV0UwMQ
	server.post("/push/send", function(req, res, next){
		var data = req.body;
		if(data) {
			push.sendAPN(data.device, data.message);
			push.sendMPNS(data.device, data.message);
		}
