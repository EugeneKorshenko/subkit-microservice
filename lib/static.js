...
module.exports.init = function(server, config, helper){
	...
	server.post("/static/upload", helper.apiAuth, function (req, res, next) {
	...
	server.post("/static/upload/:name", helper.apiAuth, function (req, res, next) {
	...
	server.put("/static/upload/:name", helper.apiAuth, function (req, res, next) {
		var fileName = req.params.name,
			fileData = req.body;
	...
	server.get("/static/download/:name", helper.apiAuth, function (req, res, next) {
		var fileName = req.params.name
			filePath = path.join(config.filesPath, fileName);
	...
	server.del("/static/:name", helper.apiAuth, function(req, res, next){
		var fileName = req.params.name,
			filePath = path.join(config.filesPath, fileName);
	...
	server.get("/static/:name", helper.apiAuth, function(req, res, next){
		var fileName = req.params.name;
		var filePath = path.join(config.filesPath, fileName);
