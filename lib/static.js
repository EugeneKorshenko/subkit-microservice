...
module.exports.init = function(server, config, helper, doc){
	var maxSize = 1000000000;
	var file = require('./file-module.js').init(config);
	
	var statics_doc = doc("file", "Static files distribution operations.");
	...
	server.get("/file", helper.apiAuth, function(req,res,next){
	...
	server.get("/file/:name", helper.apiAuth, function(req, res, next){
		var fileName = req.params.name;
		var filePath = path.join(config.filesPath, fileName);
