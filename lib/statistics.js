...
module.exports.init = function(server, store, event, es, doc){
	...
	server.pre(function (req, res, next) {
		...
		server.server.getConnections(function(err, count){
			...
			store.upsert('statistics', 'raw!' + log.id, log, function(error){
				return next();
			});
		});
	});
	setInterval(function(){
		_runRawUsageAnalytics(function(err, data){
			store.upsert('statistics', 'minute!'+microtime.now(), data, function(error){
				//TODO: logger
			});
		});
	}, 60000);
	setInterval(function(){
		...
		store.upsert('statistics', 'os!'+microtime.now(), data, function(error){
			//TODO: logger
		});
	}, 60000);
	setInterval(function(){
