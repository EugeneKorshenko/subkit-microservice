...
module.exports.init = function(doc){
	...
	stores_doc.get('/stores/{name}', 'Get items by store name.', {
	    ...
	    parameters: [
	    	...
	    	{name: 'reversed', description: 'Reverse the results. (default: false)', required:false, dataType: 'boolean', paramType: 'query'},
			...
			{name: 'limit', description: 'Limit results within numeric number. (default: -1 max: 100)', required:false, dataType: 'integer', paramType: 'query'},
			{name: 'page', description: 'Current page number. (default: 0)', required:false, dataType: 'integer', paramType: 'query'},
			{name: 'groupBy', description: 'Group by key. (default: "")', required:false, dataType: 'string', paramType: 'query'},
			{name: 'where', description: 'Filter results by JSON expression in payload values.', required:false, dataType: 'string', paramType: 'query'},
	    ],
	    errorResponses:[
			{
				code: 400,
	...
	stores_doc.get('/stores/{name}/{key}', 'Get an item by store name and key.', {
	    nickname: 'Find',
		responseClass: 'Value',
	...
	stores_doc.post('/stores/{name}', 'Create an item in store.', {
		nickname: 'Create',
		responseClass: 'void',
		note: 'curl -H "accept: application/json" -H "x-auth-token: {APIKEY}" -d {"key":"value"} -X POST {BASEURI}/stores/{name}',
		parameters: [
			{name: 'name', description: 'Name of store.', required:true, dataType: 'string', paramType: 'path'},
	...
	stores_doc.post('/stores/{name}/{key}', 'Create an item with client side key in store.', {
		nickname: 'Create',
		responseClass: 'void',
