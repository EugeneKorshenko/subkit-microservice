...
module.exports.init = function(server, storage, helper, doc){
	...
	server.post(/stores\/([a-zA-Z0-9_\.~-]+)\/(.*)/, helper.setNewETag, function (req, res, next) {
		var resource = req.params[0],
			key = req.params[1],
			payload = req.body;
	...
	server.put(/stores\/([a-zA-Z0-9_\.~-]+)\/(.*)/, helper.setNewETag, function (req, res, next) {
		var resource = req.params[0],
			key = req.params[1],
			payload = req.body;
	...
	server.del(/stores\/([a-zA-Z0-9_\.~-]+)\/(.*)/, helper.setNewETag, function (req, res, next) {
		var resource = req.params[0],
			key = req.params[1];
