...
module.exports.init = function(server, storage, helper, doc){
	...
	stores_doc.head('/stores/{name}', 'Gets all items metadata by store name.', {
	    nickname: 'FindAll',
		responseClass: 'List[Value]',
	    parameters: [
	    	{name: 'name', description: 'Start letters of name of store.', required:true, dataType: 'string', paramType: 'path'}
	...
	stores_doc.head('/stores/{name}/{key}', 'Gets item metadata.', {
	    nickname: 'Find',
		responseClass: 'Value',
		parameters: [
			{name: 'name', description: 'Name of store.', required:true, dataType: 'string', paramType: 'path'},
	...
	stores_doc.get('/stores/schema', 'Load JSON schema for a specified store name.', {
	    nickname: 'getSchema',
		responseClass: 'Value',
		parameters: [
	...
	server.head(/stores\/([a-zA-Z0-9_\.~-]+)(\/)?(.*)/, helper.apiAuth, function (req, res, next) {
		var resource = req.params[0],
			key = req.params[2];
		if(resource === '') return res.send(404, new Error('Parameter not set.'));
		res.send(200);
