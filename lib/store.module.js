...
module.exports.init = function(conf){
	...
	var query = function(resource, options, queryString, callback){
		...
		if(!options.key) {
			...
			var stream = (options.page && options.page) !== 0 ? _paginate(db, resource, { page : options.page, num : options.limit }) : db.readStream(options);
			stream
			.on('data', function (data) {
			    ...
			    try {
					if(jsonquery.match(element.$payload, queryString)) elements.push(element);
				}catch(e){}
			})
	...
	var insert = function(resource, key, payload, callback){
	...
	var update = function(resource, key, payload, callback){
		payload.$version = microtime.now();
		payload.$timestamp = new Date();
		db.put(resource + '!' + key, payload, {sync : true}, function(error, data){
	...
	var tryUpdate = function(resource, key, payload, callback){
