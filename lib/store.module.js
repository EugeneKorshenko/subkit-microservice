...
var file 		 = require('./file.module.js').init({});
...
module.exports = function (config, logger) {
	module.exports.init(config, logger);
};
...
module.exports.init = function(conf, logger){
	...
	if(!fs.existsSync(config.backupPath)) utils.mkdirRecursive(config.backupPath);
	...
	return {
		...
		* @param {String} JSONquery - JSON Query
		...
		* @param {String} resource - Name of store
		* @param {String} key - Store item key
		* @param {Number} version - Expected document version
		* @param {Object} payload - Document object
		* @param {callback} callback - Done handler
		*/
		tryUpdate: tryUpdate,
	...
	function query(resource, options, JSONquery, callback){
		...
		if(!options.key) {
			...
			.on('data', function (data) {
			    ...
			    try {
					if(jsonquery.match(element.$payload, JSONquery)) {
			...
			.on('end', function () {
				if(options.groupBy){
					var grouped = elements.reduce(function(previousValue, currentValue){			  				
							var groupKey = _(currentValue.$payload).valueForKeyPath(options.groupBy);
			  				if(!_.isArray(groupKey)){
			  					...
			  					if(jsonquery.match(currentValue.$payload, JSONquery)) previousValue[groupKey].push(currentValue);
							} else {
								if(jsonquery.match(currentValue.$payload, JSONquery)) {
	...
	function insert(resource, key, payload, callback){
		key = key || uuid.v1();
		var version = microtime.now();
		var timestamp = new Date().toISOString();
		payload.$version = version;
		payload.$timestamp = timestamp;
		db.put(resource + '!' + key, payload, {sync : true}, function(error){
			if(callback) callback(error, {key: key, store: resource, version: version, timestamp: timestamp});
		});
	}	
	function update(resource, key, payload, callback){
		...
		db.get(resource + '!' + key, function(err, data){
			...
			if(!data && callback) return callback(new Error('Not found.'));
	...
	function tryUpdate(resource, key, version, payload, callback){
		...
		db.get(resource + '!' + key, function(err, data){
			...
			var timestamp = new Date().toISOString();
			var newVersion = microtime.now();
			var newPayload = JSON.parse(JSON.stringify(payload));
			
			newPayload.$version = newVersion;
			newPayload.$timestamp = timestamp;

			db.put(resource + '!' + key, newPayload, {sync : true}, function(error){
				if(callback) callback(error, {key: key, store: resource, version: newVersion, timestamp: timestamp});
			});
		});
	}
	function del(resource, key, callback){
		...
		query(resource, {}, {}, function(err, data){
			...
			data.results.forEach(function(item){
				db.del(resource + '!' + item.$key, {sync : true}, function(error){
					if(error) return callback(error);
	...
	function backup(name, done){
		...
		var fromPath = path.join(process.cwd(), 'files');
		var toPath = path.join(config.backupPath, date) + '.tar';

		file.tarDirectory(fromPath,toPath, function(){
			done(null, date + '.tar');
		});
	}
	function restore(name, done){
		var archivePath = path.join(config.backupPath, name);
		var restorePath = path.join(process.cwd(), 'files');
		file.extractTar(archivePath, restorePath, done);
	}
	function listBackups(done){
		file.readDir(config.backupPath, done);
	}
