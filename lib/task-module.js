...
module.exports.init = function(conf, storage, pubsub, email, notify, es){
	...
	var _Task = function(task){
		task.onTick = function(){
			var self = this;
			_runPlugin(task.taskName, task.payload, function(err, data){
	...
	//tasks
	function _runPlugin(resource, params, callback){
		pluginRunner.context.params = params;
		pluginRunner.context.onData(callback);
		pluginRunner.onScriptError(callback);
		pluginRunner.run(resource);
		if(global.gc) global.gc();
	};
	function _execPlugin(resource, params, callback){
		pluginRunner.context.params = params;
		pluginRunner.context.onData(callback);
		pluginRunner.onScriptError(callback);
		var env = pluginRunner.build();
		env.load(resource);
		return env.exec();
	};
	var pluginRunner = tasks.init({
	...
	var _scheduleMapReduce = function(callback){
		fs.readdir(config.mapreducePath, function(err, files){
