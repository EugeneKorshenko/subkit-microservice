...
module.exports.init = function(server, task){
	...
	server.get(/task\/run\/([a-zA-Z0-9_\.~-]+)/, function (req, res, next) {
	...
	server.put(/task\/run\/([a-zA-Z0-9_\.~-]+)/, function (req, res, next) {
		var resource = req.params[0];
		var timeOutRef = setTimeout(function(){
			return res.send(400, new Error(resource + ' do not done. Timeout.'));
		}, 60000);
	...
	server.post(/task\/run\/([a-zA-Z0-9_\.~-]+)/, function (req, res, next) {
		var resource = req.params[0];
		var timeOutRef = setTimeout(function(){
			return res.send(400, new Error(resource + ' do not done. Timeout.'));
		}, 60000);
