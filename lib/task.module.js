...
module.exports.init = function(conf, storage, event, es, template, file, doc){
	...
	var _startSchedule = function(task, callback, force){		
		...
		if(task.Schedule){
			...
			scheduledTasks[task.Name] = new CronJob(
				task.Schedule,
				function(){ 
					event.publish('log!task',task.Name, {name: task.Name, stage:'scheduled', status:'start', timestamp: new Date().toString() });
					_run(task.Name, task.Parameters, function(){
						event.publish('log!task',task.Name, {name: task.Name, stage:'scheduled', status:'end', timestamp: new Date().toString() });
		...
		if(task.isContinuous){
			...
			event.publish('log!task',task.Name, {name: task.Name, stage:'continuous', status:'start', timestamp: new Date().toString() });
			...
			_run(task.Name, task.Parameters, function(){
				event.publish('log!task',task.Name, {name: task.Name, stage:'continuous', status:'end', timestamp: new Date().toString() }); 
				if (callback) callback(null, {
					message: task.Name + ': Task run done.',
					timestamp: new Date().toString() 
				});
	...
	var _run = function(name, params, callback){
		...
		runner.context.onResponse(function(error, data, contentType){
			event.publish('log!task',name, {name: name, stage:'task', status:'done', timestamp: new Date().toString()});
			callback(error, data, contentType, outLog);
		}, function(text){
			outLog.push(text);
		});
		runner.onScriptError(function(error, data, contentType){
			event.publish('log!task',name, {name: name, stage:'task', status:'error', message: error, timestamp: new Date().toString()});
			callback(error, data, contentType, outLog);
		});
		event.publish('log!task',name, {name: name, stage:'task', status:'run', timestamp: new Date().toString()});
		runner.run(name);
