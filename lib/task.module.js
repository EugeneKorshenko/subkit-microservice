...
module.exports.init = function(conf, storage, event, es, template, file, doc){
	...
	function startSchedule(task, callback, force){		
		...
		if(task.schedule){
			...
			utils.log(task.name + ': Cron task running by ' + task.schedule);
			scheduledTasks[task.name] = new CronJob(
				task.schedule,
				function(){ 
					utils.log('Task execution', {name: task.name, stage:'scheduled', status:'start'});
					run(task.name, task.parameters, null, function(){
						utils.log('Task execution', {name: task.name, stage:'scheduled', status:'end'});
		...
		if(task.continuous){
			utils.log(task.name + ': Continuous task running.');
			utils.log('Task execution', {name: task.name, stage:'continuous', status:'start'});
			...
			run(task.name, task.parameters, null, function(){
				utils.log('Task execution', {name: task.name, stage:'continuous', status:'end'}); 
				if (callback) callback(null, {
					message: task.name + ': Task run done',
					timestamp: new Date().toISOString() 
				});
	...
	function run(name, params, httpContext, callback){
		...
		runner.context.onDone(function(error, data, contentType){
			utils.log('Task execution', {task: name, stage:'task', status:'done'});
			callback(error, data, contentType, outLog);
		}, function(text){
			outLog.push(text);
		});
		runner.onScriptError(function(error, data, contentType){
			utils.log('Task execution', {task: name, stage:'task', status:'error', message: error});
			callback(error, data, contentType, outLog);
		});
		utils.log('Task execution', {task: name, stage:'task', status:'run'});
		runner.run(name);
