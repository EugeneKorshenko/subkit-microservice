...
module.exports.init = function(server, storage, tasksConfig, file, task, helper){
	var maxSize = 100000;
	...
	//curl -i -F filedata=@demo.js http://localhost:8080/task/upload
	server.post("/task/upload", helper.apiAuth, function (req, res, next) {
		if(!req.files) return res.send(400);

		var taskName = req.files.filedata.name,
			oldPath = req.files.filedata.path,
			newPath = path.join(tasksConfig.filesPath, taskName);
