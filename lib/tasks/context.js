...
module.exports.init = function(context, logger){
	var store = {
		...
		add: function(resource, key, payload, callback){
			var deferred = q.defer();
			context.store.add(resource, key || uuid.v1(), payload, function(error, data){
				if(error) deferred.reject(error);
				else deferred.resolve(data);
			});
			return deferred.promise.nodeify(callback);
	...
	return {
		onDone: function(doneCallback){
			...
			this.task.util = util;
			this.task.debug = function(message, persistence){
				event.emit(that.task.name, message, {}, persistence);
			};

			this.task.done = doneCallback;
			this.task.next = function(){};
			this.task.info = console.log;
			this.task.log = logger.log;
