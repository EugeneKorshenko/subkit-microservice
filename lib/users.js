...
module.exports.init = function(server, storage, helper){
	...
	server.get("/users/groups", helper.apiAuth, function(req, res, next){
	...
	server.get("/users/groups/users", helper.apiAuth, function(req, res, next){
		storage.read("users", {}, function(err, data){
			if(err) return res.send(500, err);
			var groups = data.reduce(function(state, event){
				var userGroups = event.value.groups || [];
	...
	server.get("/users/:groupId", helper.apiAuth, function(req, res, next){
	...
	server.post("/users/groups/:groupId/:userId", helper.apiAuth, function(req, res, next){
		var userId = req.params.userId;
		var groupId = req.params.groupId;
	...
	server.del("/users/groups/:groupId/:userId", helper.apiAuth, function(req, res, next){
		var userId = req.params.userId;
		var groupId = req.params.groupId;
	...
	server.get("/users/:userid", helper.apiAuth, function(req, res, next){
		var userId = req.params.userid;
	...
	server.get("/users", helper.apiAuth, function(req, res, next){
	...
	server.post("/users/:userid", helper.apiAuth, function(req, res, next){
		var userId = req.params.userid;
		var user = req.body;
	...
	server.put("/users/:userid", helper.apiAuth, function(req, res, next){
		var userId = req.params.userid;
		var user = req.body;
		user.timestamp = new Date();
		user.userid = userId;
	...
	server.del("/users/:userid", helper.apiAuth,  function(req, res, next){
		var userId = req.params.userid;
