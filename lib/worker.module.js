...
*/
module.exports = function (config, storage, hook, es, template, file, doc) {
	module.exports.init(config, storage, hook, es, template, file, doc);
};
module.exports.init = function(conf, storage, hook, es, template, file, doc){
	...
	var _init = function(workerConfig){
		...
		worker_doc = doc ? doc('worker', 'Worker operations.') : { get: function(){} };
		return self;
	};
			...
			data.forEach(function(itm){
				_get(itm.Name, function(error, task){
					...
					if(task.Name && task.Description && task.Documentation)
						worker_doc.get('/worker/run/' + task.Name, task.Description, task.Documentation);
				});
			});
		});
	};
	var _startSchedule = function(task, callback, force){		
		...
		if(task.Schedule){
			...
			scheduledTasks[task.Name] = new CronJob(
				task.Schedule,
				function(){ 
					hook.publish('log!task',task.Name, {name: task.Name, stage:'scheduled', status:'start', timestamp: new Date().toString() });
					_run(task.Name, task.Parameters, function(){
						hook.publish('log!task',task.Name, {name: task.Name, stage:'scheduled', status:'end', timestamp: new Date().toString() });
		...
		if(task.isContinuous){
			...
			hook.publish('log!task',task.Name, {name: task.Name, stage:'continuous', status:'start', timestamp: new Date().toString() });
			...
			_run(task.Name, task.Parameters, function(){
				hook.publish('log!task',task.Name, {name: task.Name, stage:'continuous', status:'end', timestamp: new Date().toString() }); 
				if (callback) callback(null, {
					message: task.Name + ': Task run done.',
					timestamp: new Date().toString() 
				});
	...
	var _run = function(name, params, callback){
		...
		runner.context.onResponse(function(error, data, contentType){
			hook.publish('log!task',name, {name: name, stage:'task', status:'done', timestamp: new Date().toString()});
			callback(error, data, contentType, outLog);
		}, function(text){
			outLog.push(text);
		});
		runner.onScriptError(function(error, data, contentType){
			hook.publish('log!task',name, {name: name, stage:'task', status:'error', message: error, timestamp: new Date().toString()});
			callback(error, data, contentType, outLog);
		});
		hook.publish('log!task',name, {name: name, stage:'task', status:'run', timestamp: new Date().toString()});
		runner.run(name);
