...
module.exports.init = function(conf, storage, pubsub, es){
	...
	var _loadByName = function(name, callback){
	...
	var _runScheduler = function(callback){
		if(!callback) callback = function(error, data){
			pubsub.publish('log!task',uuid.v1(), {stage:'schedule', status:'done', payload: error || data});
		};
		
		_list(function(error, data){
			data.forEach(function(itm){
				_loadByName(itm.Name, function(error, task){					
	...
	var _startSchedule = function(task, callback){
		...
		if(!task.Schedule || task.Schedule === '') return;
	...
	//tasks
	var _run = function(resource, params, callback){
		var outLog = [];
	...
	var _exec = function(resource, params, callback){
		runner.context.params = params;
	...
	var _run2 = function(resource, params, callback){		
		var outLog = [];
		runner.context.params = params;
		runner.context.onData(function(error, data, contentType){
			pubsub.publish('log!task',uuid.v1(), {stage:'task', status:'end'});
