//storage
server.get("/stores", apiAuth, function (req, res, next) {
...
server.post("/store/:name/grant", userAuth, setNewETag, function (req, res, next) {
...
server.del("/store/:name/grant", userAuth, setNewETag, function (req, res, next) {
	var storeName = req.params.name;
	if(!storeName) return res.send(404, new Error("Parameters not set."));
...
server.get(/store\/([a-zA-Z0-9_\.~-]+)(\/)?(.*)/, apiAuth, function (req, res, next) {
...
server.post(/store\/([a-zA-Z0-9_\.~-]+)\/(.*)/, apiAuth, setNewETag, function (req, res, next) {
	var resource = req.params[0]
		,key = req.params[1]
		,payload = req.body;
...
server.put(/store\/([a-zA-Z0-9_\.~-]+)\/(.*)/, apiAuth,setNewETag, function (req, res, next) {
	var resource = req.params[0]
		,key = req.params[1]
		,payload = req.body;
...
server.del(/store\/([a-zA-Z0-9_\.~-]+)\/(.*)/, apiAuth, setNewETag, function (req, res, next) {
	var resource = req.params[0]
		,key = req.params[1];
