...
server.get("/tasks", apiAuth, function(req,res,next){
...
server.post("/task/upload", userAuth, function (req, res, next) {
...
server.post("/task/upload/:name", userAuth, function (req, res, next) {
...
server.put("/task/upload/:name", userAuth, function (req, res, next) {
	var fileName = req.params.name,
		file = req.body.toString();
...
server.get("/task/download/:name", userAuth, function (req, res, next) {
	var fileName = req.params.name
		filePath = path.join(storageModule.tasksPath, fileName);
...
server.del("/task/:name", userAuth, function(req, res, next){
	var fileName = req.params.name,
		filePath = path.join(storageModule.tasksPath, fileName);
...
server.post("/task/:name", userAuth, function(req, res, next){
	var fileName = req.params.name,
		file = req.body,
		filePath = path.join(storageModule.tasksPath, fileName);
	if(file > 100000) return res.send(400);
...
server.put("/task/:name", userAuth, function(req, res, next){
	var fileName = req.params.name,
		file = req.body,
		filePath = path.join(storageModule.tasksPath, fileName);
	if(file > 100000) return res.send(400);
...
//run tasks
server.get(/run\/([a-zA-Z0-9_\.~-]+)/, apiAuth, function (req, res, next) {
...
server.post(/run\/([a-zA-Z0-9_\.~-]+)/, apiAuth, function (req, res, next) {
	var resource = req.params[0];
